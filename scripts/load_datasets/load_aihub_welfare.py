...
def process_single_audio(wav_path_obj):
    ...
    try:
        ...
        # json 읽기
        json_path = abs_wav_path.with_suffix('.json')
        if not json_path.exists():
            return None
            
        with open(json_path, 'r', encoding='utf-8') as f:
            label_data = json.load(f)
            input_text_list = label_data.get('inputText', [])
            if not input_text_list:
                return None
            text_content = input_text_list[0].get('orgtext', "").strip()
...
def create_sampled_dataset(root_dir, output_csv, total_target=300_000):
    root_path = Path(root_dir)
    target_dirs = ['01.대학병원', '02.광역이동지원센터', '03.정신건강복지센터']
...
# --- 설정 ---
ROOT_DIRECTORY = "/home/data/bootcamp/team/data_aihub/186.복지_분야_콜센터_상담데이터/01.데이터/1.Training" # 1.Training에 대해서도 실행하기, 아니면 01.데이터에 대해서 해도 됨
OUTPUT_FILE = "ai_hub_welfare_sampled_train.csv"
